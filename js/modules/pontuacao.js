...
function aplicarEstilosUXPontuacao() {
  ...
  style.textContent = `
    ...
    .lancamento-help{font-size:.82rem;color:var(--text-light);margin:8px 0 0;line-height:1.35}
    ...
    .lote-stats{display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin-bottom:12px}
    ...
    .lote-stat strong{display:block;color:var(--text);font-size:1rem}
    .lote-stat span{font-size:.68rem;color:var(--text-light)}
    .lote-actions{display:flex;gap:8px;align-items:center;justify-content:space-between}
    .lote-details{display:none;border-top:1px solid var(--border);padding:10px 16px 14px;background:rgba(0,0,0,.015);max-height:220px;overflow:auto}
    .lote-card.open .lote-details{display:block}
    .lote-row{display:flex;justify-content:space-between;gap:8px;border-bottom:1px dashed var(--border);padding:7px 0;font-size:.82rem}
    ...
    .nome-atleta-link{color:var(--primary);font-weight:800;cursor:pointer;text-decoration:none;border-bottom:1px dashed rgba(0,155,193,.45)}
    .nome-atleta-link:hover{filter:brightness(.9)}
...
function reconstruirDropdownEventos() {
  ...
  eventos.forEach(e => {
    ...
    if (d >= hoje) proximos.push(e);
    else if (d >= limite) recentes.push(e);
  });
...
function setupExtratoAgrupadoUI() {
  ...
  wrap.innerHTML = `
    <div class="extrato-tabs">
      <button type="button" class="extrato-tab active" data-view="lotes"><i data-lucide="layers-3"></i> Por lançamento</button>
      <button type="button" class="extrato-tab" data-view="auditoria"><i data-lucide="list"></i> Auditoria detalhada</button>
    </div>
  ...
  wrap.querySelectorAll(".extrato-tab").forEach(btn => {
    btn.addEventListener("click", () => {
      ...
      document.getElementById("extratoLotes").style.display = view === "lotes" ? "grid" : "none";
  ...
  ["filtroMesHistorico", "filtroEquipeHistorico", "filtroNomeHistorico", "filtroStatusHistorico"].forEach(id => {
    document.getElementById(id)?.addEventListener("input", () => setTimeout(renderizarExtratoAgrupado, 50));
  });
  document.getElementById("btnLimparFiltrosExtrato")?.addEventListener("click", () => setTimeout(renderizarExtratoAgrupado, 80));

  observerHistorico = new MutationObserver(() => setTimeout(renderizarExtratoAgrupado, 50));
  observerHistorico.observe(tbody, { childList: true });
  if(typeof lucide !== 'undefined') lucide.createIcons();
}
...
function renderizarExtratoAgrupado() {
  ...
  if (dados.length === 0) {
    container.innerHTML = `<div class="empty-state" style="grid-column:1/-1;"><i data-lucide="clipboard-list"></i><p>Nenhum lançamento encontrado para os filtros selecionados.</p></div>`;
    if(typeof lucide !== 'undefined') lucide.createIcons();
    return;
  }
  ...
  container.querySelectorAll(".btn-toggle-lote").forEach(btn => {
    btn.addEventListener("click", () => btn.closest(".lote-card")?.classList.toggle("open"));
  });
...
function agruparLancamentos(dados) {
  ...
  return Array.from(mapa.values()).map(g => {
    const atletas = new Set(g.itens.map(i => i.atletaId));
    g.qtdAtletas = atletas.size;
    ...
    return g;
  }).sort((a,b) => new Date(b.dataTreino || b.criadoEm || "1970-01-01") - new Date(a.dataTreino || a.criadoEm || "1970-01-01"));
}

function criarCardLote(g) {
  const tipoLabel = { treino: "Treino", evento: "Evento", avulso: "Avulso", importacao: "Importação" }[g.tipo] || "Lançamento";
  const detalhes = g.itens.map(i => {
    const pts = Number(i.pontos) === 0 ? "Justificada" : `+${i.pontos}`;
    const nome = escapeHtml(i.atletaNome || appState.mapAtletas[i.atletaId]?.nome || "Atleta não encontrado");
    return `<div class="lote-row"><span><a class="nome-atleta-link" data-atleta-id="${i.atletaId || ''}">${nome}</a><br><small style="color:var(--text-light);">${escapeHtml(i.regraDesc || "-")}</small></span><strong style="color:${Number(i.pontos) === 0 ? 'var(--accent)' : 'var(--secondary)'};">${pts}</strong></div>`;
  }).join("");
    ...
    <div class="lote-card">
      <div class="lote-card-head">
        <div>
          ...
          <div class="lote-meta"><span class="lote-badge">${tipoLabel}</span> ${escapeHtml(g.equipe || "-")} · ${escapeHtml(g.criadoPorNome || "Comitê")}</div>
        </div>
      ...
      <div class="lote-body">
        ...
        <div class="lote-actions">
          ...
          <button type="button" class="btn-acao btn-toggle-lote"><i data-lucide="chevron-down"></i> Detalhes</button>
        </div>
      </div>
...
function tentarAbrirFichaAtleta(atletaId) {
  ...
  if (btn) btn.click();
  else showToast("Abra a ficha pela tela de Equipes para ver todos os detalhes deste atleta.", "info");
}
...
async function salvarPontuacoesEmLote() {
  ...
  mostrarConfirmacao("Gravar Lançamentos", resumo, async () => {
    ...
    try {
      ...
      const dadosLote = {
        loteId, tipoLancamento, tituloLancamento: desc, modalidade,
        criadoPor: appState.currentUser?.uid || "", criadoPorNome: meuNome, criadoEm: new Date().toISOString()
      };
      ...
      for (let f of checksFaltas) {
        batch.set(doc(collection(db, "historico_pontos")), {
          ...
          eventoId: eventoIdSelecionado, ...dadosLote
        });
      }
      ...
      for (let check of checksPontos) {
        ...
        batch.set(doc(collection(db, "historico_pontos")), {
          ...
          descTreino: desc, dataTreino: data, eventoId: eventoIdSelecionado, ...dadosLote
        });
      ...
      lancamentosValidos.forEach(l => {
        ...
        if (appState.mapAtletas[aId]) {
          batch.set(doc(collection(db, "historico_pontos")), {
            ...
            loteId, tipoLancamento: "importacao", tituloLancamento: desc,
...
function gerarLoteId(tipo) {
  ...
  return `${tipo}_${Date.now()}_${rand}`;
}

function rotuloTipo(tipo) {
  return { treino: "Treino", evento: "Evento", avulso: "Avulso", importacao: "Importação" }[tipo] || "Lançamento";
}
...
function formatarData(dataStr) {
  if (!dataStr) return "-";
  try { return new Date(dataStr + "T00:00:00").toLocaleDateString('pt-BR'); }

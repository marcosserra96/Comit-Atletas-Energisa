...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .sheet {
    ...
    min-height:900px;
    margin:0 auto;
    background:radial-gradient(circle at 20% 0%, rgba(0,169,200,.18), transparent 30%), var(--navy);
    position:relative;
    overflow:hidden;
  ...
  .title h1 { margin:2px 0 0; font-size:34px; line-height:1.18; letter-spacing:.4px; }
  .title p { margin:8px 0 0; color:var(--muted); font-size:16px; }
  .legend { justify-self:end; padding-top:18px; font-size:18px; }
  .legend div { display:flex; align-items:center; gap:14px; margin-bottom:12px; white-space:nowrap; }
  .legend span { display:block; width:140px; height:14px; border-radius:2px; }
  ...
  .bike-icon { position:absolute; left:110px; top:-7px; font-size:44px; }
  .tables { display:grid; grid-template-columns:1fr 1fr; gap:26px; margin-left:54px; }
  table { width:100%; border-collapse:collapse; table-layout:fixed; font-size:18px; }
  th {
    height:48px;
    background:#00a7bd;
    color:white;
  ...
  .trophy { position:absolute; left:0; top:338px; width:86px; text-align:center; font-size:56px; filter:drop-shadow(0 6px 10px rgba(0,0,0,.35)); }
...
function montarTabelaRankingInformativo(titulo, lista) {
  const linhas = lista.slice(0, 28).map((a, idx) => {
    const classe = idx < 3 ? "top3" : (a.pontosMes <= 0 && a.treinosMes <= 0 ? "alert" : "normal");

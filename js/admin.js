...
// AQUI ESTÁ A CORREÇÃO DE ORDEM DO DASHBOARD FINANCEIRO E DE ATLETAS
async function atualizarTelas() {
  if (userRole === "admin" || userPermissoes.includes("gestao")) {
    setupAprovacoes();
  }
  ...
  mapAtletas = {}; 
  snapA.forEach(d => { 
    mapAtletas[d.id] = { id: d.id, ...d.data() }; 
  });

  await carregarHistorico(); 
  
  // 1. Carrega o Financeiro primeiro (preenche o gastoTotalGlobal)
  await carregarFinanceiroPlanilha(); 
  
  // 2. Só depois processa as equipas e o dashboard de ROI
...
function setupFinanceiroPlanilha() {
  ...
  document.getElementById("salvarVerbaBtn")?.addEventListener("click", async (e) => {
    ...
    try {
      ...
      showToast("Erro ao gravar: " + (err.message || "Tente novamente"), "error"); 
    }
    ...
    document.getElementById("salvarEventoBtn").addEventListener("click", async (e) => {
      ...
      try {
        await addDoc(collection(db, "agenda_eventos"), { 
          titulo: titulo, local: local, modalidade: mod, data: data, criadoEm: new Date().toISOString() 
        });
        modal.style.display = "none"; 
      ...
      e.target.textContent = "Salvar Evento"; 
      e.target.disabled = false; 
    });
  }
}
...
async function carregarEquipesEDashboard() {
  ...
  // Ações diversas (Pular, Excluir, Editar)
    ...
    btn.addEventListener("click", async (e) => { 
      ...
      if(confirm(`Passar a vez do atleta? Ele trocará de posição com o próximo da fila.`)) { 
        ...
        try {
          ...
          showToast("Erro ao pular fila: " + err.message, "error");
        }
      }
    }); 
  });
  ...
  document.querySelectorAll(".toggle-ativo").forEach(chk => { 
    chk.addEventListener("change", async (e) => { 
      ...
      try {
        ...
        if(isAtivo) td.classList.remove('inativo-txt'); 
        else td.classList.add('inativo-txt'); 
...
function setupPermissoesModal() {
  ...
  document.getElementById("salvarPermissoesBtn").addEventListener("click", async (e) => {
    ...
    try {
      ...
      showToast("Erro a salvar permissões.", "error");
    }
    
    e.target.textContent = "Salvar Acessos"; 
    e.target.disabled = false; 
  });
}
...
async function abrirFichaAtleta(id) {
  ...
  if(a.ativo !== false) { 
    statusEl.textContent = "Ativo no Sistema"; 
  ...
  hist.forEach(h => { 
    ...
    htmlH += `<div style="border-bottom: 1px solid var(--border); padding: 8px 0; display:flex; justify-content:space-between; align-items:center;">
...
async function renderGraficosETop(ptsBike, ptsCorrida, arrayAtletas, totalBike, totalCorrida) {
  ...
  if(document.getElementById('graficoEngajBike')) { 
    ...
    graficoEngajBike = new Chart(document.getElementById('graficoEngajBike'), { 
      type: 'doughnut', 
  ...
  if(document.getElementById('graficoEngajCorrida')) { 
    ...
    graficoEngajCorrida = new Chart(document.getElementById('graficoEngajCorrida'), { 
      type: 'doughnut', 
  ...
  document.getElementById("modTreino").addEventListener("change", async (e) => { 
    ...
    try {
      ...
      snapRegras.forEach(d => { 
        const r = d.data(); 
        regrasArray.push({ id: d.id, descricao: r.descricao, pontos: r.pontos }); 
      }); 
...
async function gerarTabelaContabilizacao(modalidade, regras) {
  ...
  let thead = `<thead><tr>
                 ...
                 <th style="text-align:center; color:var(--secondary); vertical-align:middle; min-width: 90px; border-right: 2px solid var(--border);">
                   <div style="display:flex; flex-direction:column; align-items:center; gap:5px;">
                     <span style="font-weight:bold; font-size: 0.8rem;">Cumpriu Todas</span>
                     <label style="font-size:0.75rem; display:flex; align-items:center; gap:4px; cursor:pointer; margin:0;"><input type="checkbox" id="checkMasterCumpriu"> Todo Time</label>
                   </div>
                 </th>`;
  ...
  atletas.forEach(a => {
    thead += `<tr>
                <td style="vertical-align:middle; font-weight:500;">${a.nome}</td>
                <td style="text-align:center; background: rgba(243,112,33,0.05); vertical-align:middle; border-left: 1px solid var(--border);">
                  <input type="checkbox" class="check-falta" data-atleta-id="${a.id}" data-atleta-nome="${a.nome}" data-atleta-equipe="${a.equipe}">
                </td>
                <td style="text-align:center; background: rgba(0,179,126,0.05); vertical-align:middle; border-right: 2px solid var(--border);">
                  <input type="checkbox" class="check-cumpriu" data-atleta-id="${a.id}">
                </td>`;
    regras.forEach(r => { 
      thead += `<td style="text-align:center; vertical-align:middle;">
                  <input type="checkbox" class="check-ponto" data-atleta-id="${a.id}" data-atleta-nome="${a.nome}" data-atleta-equipe="${a.equipe}" data-regra-id="${r.id}" data-regra-desc="${r.descricao}" data-pontos="${r.pontos}">
                </td>`; 
    });
    thead += `</tr>`;
  }); 
  ...
  document.getElementById("checkMasterCumpriu").addEventListener("change", (e) => { 
    document.querySelectorAll(".check-cumpriu").forEach(chk => { 
      if(!chk.disabled) { chk.checked = e.target.checked; chk.dispatchEvent(new Event('change')); } 
    }); 
  });
  
  document.getElementById("checkMasterFalta").addEventListener("change", (e) => { 
    document.querySelectorAll(".check-falta").forEach(chk => { 
      chk.checked = e.target.checked; chk.dispatchEvent(new Event('change')); 
    }); 
  });
  ...
  document.querySelectorAll(".check-falta").forEach(chk => { 
    chk.addEventListener("change", (e) => { 
      ...
      const cCumpriu = tr.querySelector(".check-cumpriu"); 
      cCumpriu.disabled = e.target.checked; 
  ...
  document.querySelectorAll(".check-cumpriu").forEach(chk => { 
    chk.addEventListener("change", (e) => { 
      const tr = e.target.closest("tr"); 
      tr.querySelectorAll(".check-ponto").forEach(p => { 
        if(!p.disabled) p.checked = e.target.checked; 
      }); 
    }); 
  });
}
  ...
  try {
    ...
    for (let f of checksFaltas) { 
      batch.set(doc(collection(db, "historico_pontos")), { 
        ...
        regraId: "falta_just", regraDesc: "Falta Justificada", pontos: 0, 
    ...
    await batch.commit(); 
    showToast("Lançamentos registados com sucesso!", "success"); 
    document.getElementById("areaTabelaPontuacao").style.display = "none"; 
    document.getElementById("descTreino").value = ""; 
    ...
    showToast("Erro ao processar o lote: " + error.message, "error"); 
  } finally { 
    btn.innerHTML = `Salvar Lançamentos em Lote`; 
    btn.disabled = false; 
  }
}
...
function setupRelatorioConsolidado() {
  ...
  document.getElementById("btnExportarExcel").addEventListener("click", () => {
    ...
    if(tbody.innerText.includes("Clique em Filtrar") || tbody.innerText.includes("Nenhum atleta")) return showToast("Gere primeiro o relatório!", "error");
...
async function setupAprovacoes() {
  ...
  try {
    ...
    console.error("Erro a carregar aprovações", err);
  }
}
  ...
  document.getElementById("btnCadastrarPessoa").addEventListener("click", async (e) => {
    ...
    try {
      ...
      await addDoc(collection(db, "atletas"), { 
        nome: nome, email: email, role: "atleta", equipe: papel, 
        status: "Aprovado", ativo: true, pontuacaoTotal: 0, recusas: 0, 
...
async function carregarRegras() {
  ...
  try {
    ...
    snap.forEach(d => { 
      const r = d.data(); 
      const btnExcluir = canEdit ? `<button class="btn-acao btn-excluir-regra" data-id="${d.id}" style="color:var(--danger); border-color:var(--danger);"><i data-lucide="trash" style="width:16px;"></i></button>` : ''; 
      tbody.innerHTML += `<tr>
...
function setupModalRegras() {
  ...
  document.getElementById("salvarRegraBtn").addEventListener("click", async (e) => {
    ...
    try {
      ...
      showToast("Erro a criar regra: " + err.message, "error");
    }
...
function setupConfiguracoes() {
  ...
  document.getElementById("btnSalvarConta").addEventListener("click", async () => {
    const senhaAtual = document.getElementById("confSenhaAtual").value;
    const novoEmail = document.getElementById("confNovoEmail").value.trim();
    const novaSenha = document.getElementById("confNovaSenha").value.trim();
